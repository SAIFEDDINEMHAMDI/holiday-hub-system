...
const Dashboard = () => {
  ...
  const totalLeaveDays = Object.values(user.leaveBalance).reduce((sum, days) => sum + days, 0);
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-primary-50 rounded-lg">
              <div className="text-2xl font-bold text-primary-700">{user.leaveBalance.annual}</div>
              <div className="text-sm text-primary-600">Congés Annuels</div>
            </div>
            <div className="text-center p-4 bg-secondary-50 rounded-lg">
              <div className="text-2xl font-bold text-secondary-700">{user.leaveBalance.sick}</div>
              <div className="text-sm text-secondary-600">Congés Maladie</div>
            </div>
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <div className="text-2xl font-bold text-purple-700">{user.leaveBalance.personal}</div>
                ...
                <div key={request.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {request.leaveType} - {request.startDate} au {request.endDate}
                    </div>
                  </div>
