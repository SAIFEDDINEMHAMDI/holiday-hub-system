...
export const generateLeavePDF = (request: LeaveRequest) => {
  ...
  doc.text('Nom:', 20, 65);
  doc.setFont('helvetica', 'normal');
  doc.text(request.employeeName, 50, 65);
  
  doc.setFont('helvetica', 'bold');
  doc.text('ID Employé:', 20, 75);
  doc.setFont('helvetica', 'normal');
  doc.text(request.employeeId, 50, 75);
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(getLeaveTypeLabel(request.leaveType), 70, 110);
  
  doc.setFont('helvetica', 'bold');
  doc.text('Date de début:', 20, 120);
  doc.setFont('helvetica', 'normal');
  doc.text(new Date(request.startDate).toLocaleDateString('fr-FR'), 70, 120);
  
  doc.setFont('helvetica', 'bold');
  doc.text('Date de fin:', 20, 130);
  doc.setFont('helvetica', 'normal');
  doc.text(new Date(request.endDate).toLocaleDateString('fr-FR'), 70, 130);
  ...
  if (request.managerComment) {
    ...
    doc.setFont('helvetica', 'normal');
    const commentLines = doc.splitTextToSize(request.managerComment, 150);
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`Date de demande: ${new Date(request.appliedDate).toLocaleDateString('fr-FR')}`, 20, finalYPos + 15);
  ...
  // Télécharger le PDF
  const fileName = `conge_${request.employeeName.replace(/\s+/g, '_')}_${request.startDate}.pdf`;
  doc.save(fileName);
};
